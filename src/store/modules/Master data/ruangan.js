...
const ruangan = {
  ...
  mutations: {
    ...
    SET_DATA_KOMPUTER_RUANGAN(state, dataKomputerByRuangan) {
      state.dataKomputerRuangan = dataKomputerByRuangan;
    },
  },
  actions: {
    ...
    async fetchDataKomputerRuangan({ commit }, slug) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          `https://ptipd-lab-inventory-api-production.up.railway.app/komputer?search_query=${slug}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        commit("SET_DATA_KOMPUTER_RUANGAN", response.data["data"]);
      } catch (error) {
        alert(error);
        console.log(error);
      }
  ...
  getters: {
    ...
    getDataKomputerRuangan: (state) => state.dataKomputerRuangan,
