...
const user = {
  ...
  mutations: {
    ...
    DELETE_USE(state, iduser) {
      state.datauser = state.datauser.filter((k) => k.id !== iduser);
    },
  ...
  actions: {
    async fetchDatauser({ commit }) {
      try {
        ...
        const response = await axios.get(
          "https://ptipd-lab-inventory-api-production.up.railway.app/users",
          {
            headers: {
    ...
    async tambahDatauser({ commit }, userdata) {
      try {
        console.log("Mengirim permintaan pendaftaran:", userdata);
        ...
        commit("SET_TOTAL_USER", response.data["data"].length);

        console.log("Email verifikasi berhasil dikirim:", response.data);
      } catch (error) {
    ...
    async hapusDatauser({ commit }, iduser) {
      try {
        await axios.delete(
          `https://ptipd-lab-inventory-api-production.up.railway.app/users/${iduser}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
