<template>
  <v-container>
    ...
    <v-card flat class="border mb-4">
      <div class="d-flex">
        <v-card-title class="me-auto">Data Laptop</v-card-title>
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(laptop, index) in filteredData" :key="laptop.id">
            <td>{{ index + 1 }}</td>
            <td>{{ laptop.id }}</td>
...
export default {
  ...
  return this.getDataLaptop.filter((laptop) => {
    return (
      (typeof laptop.id === 'string' && laptop.id.toLowerCase().includes(searchLowerCase)) ||
      (laptop.nama_merk && laptop.nama_merk.toLowerCase().includes(searchLowerCase)) ||
      (laptop.spek && laptop.spek.toLowerCase().includes(searchLowerCase)) ||
      (laptop.nama_ruangan && laptop.nama_ruangan.toLowerCase().includes(searchLowerCase)) ||
      (laptop.urutan_meja && laptop.urutan_meja.toLowerCase().includes(searchLowerCase)) ||
      (laptop.kondisi && laptop.kondisi.toLowerCase().includes(searchLowerCase))
    );
  });
},
