  ...
  <v-container>
    ...
    <v-card flat class="border mb-4">
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(ruangan, index) in filteredData" :key="ruangan.id">
            <td>{{ index + 1 }}</td>
            <td>{{ ruangan.id }}</td>
            ...
            <td>
              <router-link :to="'/ruangan/detail/' + ruangan.nama_ruangan">
...
export default {
  ...
  return this.getDataRuangan.filter((ruangan) => {
    return (
      (typeof ruangan.id === 'string' && ruangan.id.toLowerCase().includes(searchLowerCase)) ||
      (ruangan.nama_ruangan && ruangan.nama_ruangan.toLowerCase().includes(searchLowerCase)) ||
      (ruangan.penanggung_jawab && typeof ruangan.penanggung_jawab === 'string' && ruangan.penanggung_jawab.toLowerCase().includes(searchLowerCase)) ||
      (ruangan.jumlah_komputer_laptop && typeof ruangan.jumlah_komputer_laptop === 'string' && ruangan.jumlah_komputer_laptop.toLowerCase().includes(searchLowerCase))
    );
  });
},
  },
  methods: {
    ...
    lihatDetailKomputer(idRuangan) {
      // Panggil aksi untuk mengambil data komputer pada ruangan tertentu
      this.$store.dispatch("ruangan/fetchDataKomputerRuangan", idRuangan);
    },
    async editData() {
      try {
