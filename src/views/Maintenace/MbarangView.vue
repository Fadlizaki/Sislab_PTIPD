<template>
  <v-container>
    ...
    <v-card flat class="border mb-4">
      <div class="d-flex">
        <v-card-title class="me-auto">Data Maintenace Barang</v-card-title>
        <v-text-field
          v-model="search"
          :loading="loading"
          density="compact"
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(mbarang, index) in filteredData" :key="mbarang.id">
            <td>{{ index + 1 }}</td>
...
export default {
  computed: {
    ...
    filteredData() {
      ...
      return this.getDatabarangmain.filter((mbarang) => {
        return (
          (typeof mbarang.id_komputer === "string" &&
            mbarang.id_komputer.toLowerCase().includes(searchLowerCase)) ||
          (mbarang.nama_ruangan &&
            mbarang.nama_ruangan.toLowerCase().includes(searchLowerCase)) ||
          (mbarang.kondisi &&
            mbarang.kondisi.toLowerCase().includes(searchLowerCase)) ||
          (mbarang.jenis_perbaikan &&
            mbarang.jenis_perbaikan.toLowerCase().includes(searchLowerCase)) ||
          (mbarang.tanggal_mulai &&
            this.formatDate(mbarang.tanggal_mulai).includes(searchLowerCase)) ||
          (mbarang.tanggal_berakhir &&
            this.formatDate(mbarang.tanggal_berakhir).includes(searchLowerCase))
        );
      });
    },
    isAdmin() {
  ...
  watch: {
    ...
    convertISOtoDate(isoString, mbarang) {
      try {
        ...
        if (parsedDate.isValid()) {
          ...
          if (mbarang && parsedDate.isBefore(moment.utc(), "day")) {
            // Hapus data jika tanggal berakhir sudah lewat
            this.hapusDatabarangmain(mbarang.id);
            return "Data Expired";
          }
    ...
    async editData() {
      try {
        ...
        formEditbarangmain.tanggal_mulai = moment(
          formEditbarangmain.tanggal_mulai
        ).format("YYYY-MM-DD");
        formEditbarangmain.tanggal_berakhir = moment(
          formEditbarangmain.tanggal_berakhir
        ).format("YYYY-MM-DD");
    ...
    openDialogEdit(mbarang) {
      this.formData = {
        ...mbarang,
        tanggal_mulai: moment(mbarang.tanggal_mulai).format("YYYY-MM-DD"),
        tanggal_berakhir: moment(mbarang.tanggal_berakhir).format("YYYY-MM-DD"),
      };
