<template>
  <v-container>
    ...
    <v-card flat class="border mb-4">
      <div class="d-flex">
        <v-card-title class="me-auto">Data Tipe</v-card-title>
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(tipe, index) in filteredData" :key="tipe.id">
            <td>{{ index + 1 }}</td>
            <td>{{ tipe.id }}</td>
...
export default {
  ...
  return this.getDataTipe.filter((tipe) => {
    return (
      (typeof tipe.id === 'string' && tipe.id.toLowerCase().includes(searchLowerCase)) ||
      (tipe.tipe_barang && tipe.tipe_barang.toLowerCase().includes(searchLowerCase)) 
    );
  });
},
  },
  methods: {
    ...
    async editData() {
      try {
        ...
        await this.ubahDatatipe({ tipeId, formEdittipe }).then(async () => {
          await this.fetchDataTipe(); // Ambil data terbaru
          this.resetForm(); // Reset formulir
