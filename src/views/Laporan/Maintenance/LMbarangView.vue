  ...
  <v-container>
    ...
    <v-card flat class="border mb-4">
      <div class="d-flex justify-space-between">
        <v-card-title>Data Maintenace Barang</v-card-title>
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(LMbarang, index) in filteredData" :key="LMbarang.id">
            <td>{{ index + 1 }}</td>
...
export default {
  computed: {
    ...
    filteredData() {
      ...
      return this.getDatabarangmain.filter((LMbarang) => {
        return (
          (typeof LMbarang.id_komputer === "string" &&
            LMbarang.id_komputer.toLowerCase().includes(searchLowerCase)) ||
          (LMbarang.nama_ruangan &&
            LMbarang.nama_ruangan.toLowerCase().includes(searchLowerCase)) ||
          (LMbarang.kondisi &&
            LMbarang.kondisi.toLowerCase().includes(searchLowerCase)) ||
          (LMbarang.jenis_perbaikan &&
            LMbarang.jenis_perbaikan.toLowerCase().includes(searchLowerCase)) ||
          (LMbarang.tanggal_mulai &&
            this.formatDate(LMbarang.tanggal_mulai).includes(
              searchLowerCase
            )) ||
          (LMbarang.tanggal_berakhir &&
            this.formatDate(LMbarang.tanggal_berakhir).includes(
              searchLowerCase
            ))
        );
      });
  ...
  methods: {
    convertISOtoDate(isoString) {
      ...
      if (parsedDate.isValid()) {
        return parsedDate.locale("id").format("DD MMM YYYY");
      } else {
        return "Invalid Date";
      }
    },
    ...
    printData(LMbarang) {
      ...
      printWindow.document.write(
        "<td>" + this.convertISOtoDate(LMbarang.tanggal_mulai) + "</td>"
      );
      printWindow.document.write(
        "<td>" + this.convertISOtoDate(LMbarang.tanggal_berakhir) + "</td>"
      );
      printWindow.document.write("</tr>");
