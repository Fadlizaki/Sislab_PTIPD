  ...
  <v-container>
    ...
    <v-card flat class="border mb-4">
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(LMruangan, index) in filteredData" :key="LMruangan.id">
            <td>{{ index + 1 }}</td>
            <td>{{ LMruangan.id }}</td>
...
export default {
  computed: {
    ...
    filteredData() {
      ...
      return this.getDataruanganmain.filter((LMruangan) => {
        return (
          (typeof LMruangan.id_komputer === "string" &&
            LMruangan.id_komputer.toLowerCase().includes(searchLowerCase)) ||
          (LMruangan.nama &&
            LMruangan.nama.toLowerCase().includes(searchLowerCase)) ||
          (LMruangan.nama_ruangan &&
            LMruangan.nama_ruangan.toLowerCase().includes(searchLowerCase)) ||
          (LMruangan.tanggal_mulai &&
            this.formatDate(LMruangan.tanggal_mulai).includes(
              searchLowerCase
            )) ||
          (LMruangan.tanggal_berakhir &&
            this.formatDate(LMruangan.tanggal_berakhir).includes(
              searchLowerCase
            ))
        );
      });
  ...
  methods: {
    ...
    printData(LMruangan) {
      ...
      printWindow.document.write("<td>" + this.convertISOtoDate(LMruangan.tanggal_mulai) + "</td>");
      printWindow.document.write("<td>" + this.convertISOtoDate(LMruangan.tanggal_berakhir) + "</td>");
      printWindow.document.write("</tr>");
    ...
    printAllData() {
      ...
      dataToPrint.forEach((LMruangan, index) => {
        ...
        printWindow.document.write("<td>" + this.convertISOtoDate(LMruangan.tanggal_mulai) + "</td>");
        printWindow.document.write(
          "<td>" + this.convertISOtoDate(LMruangan.tanggal_berakhir) + "</td>"
        );
